...
function readTsConfig(ts, tsconfigPath) {
    ...
    var tsconfig = ts.readConfigFile(existingTsConfig, function (path) { return fs.readFileSync(path, 'utf8'); });
    ...
    if (tsconfigPath && extendedTsConfig) {
        tsconfig.config.extends = path.join(process.cwd(), existingTsConfig, '..', extendedTsConfig);
    }
...
var readFileAsync = function (file) {
    return new Promise(function (fulfil, reject) {
        return fs.readFile(file, 'utf-8', function (err, contents) { return (err ? reject(err) : fulfil(contents)); });
    });
};
...
function typescript(options) {
    ...
    var opts = Object.assign({}, options);
    var filter = pluginutils.createFilter(opts.include || ['*.ts+(|x)', '**/*.ts+(|x)'], opts.exclude || ['*.d.ts', '**/*.d.ts']);
    delete opts.include;
    delete opts.exclude;
